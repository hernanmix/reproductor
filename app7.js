...
function renderEvents(data) {
  ...
  data.forEach(ev => {
    ...
    if (status === "live") {
      ...
      card.onclick = () => {
        // Extraemos el canal del JSON: "go:espn" → "espn"
        let channel = ev.url.replace('go:', '');
        window.open(
          `https://hernanmix.github.io/reproductor/player1.html?channel=${channel}`,
          '_blank',
