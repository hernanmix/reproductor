...
function renderEvents(data) {
  ...
  data.forEach(ev => {
    ...
    // ⛔ eliminar 1 min después de finalizado
    if (status === "finished" && (now - endTime) > 60000) return;

    const card = document.createElement("div");
    card.className = "card";

    const progress = getProgress(ev.start, ev.end);
    ...
    // Obtener la fecha y las horas formateadas
    const formattedDate = formatDate(ev.start);
    const formattedStartTime = formatTime(ev.start);
    const formattedEndTime = formatTime(ev.end);

    // Mostrar la fecha y hora solo si el evento está PRÓXIMAMENTE
    const eventDateInfo = status === "upcoming" 
      ? `<p><small>${formattedDate} • ${formattedStartTime} a ${formattedEndTime}</small></p>`
      : '';
    ...
    card.innerHTML = `
      ...
      <div style="padding:10px">
        ...
        <h3>${ev.title}</h3>
        <p>${ev.league}</p>
        ${eventDateInfo} <!-- Mostrar la fecha y hora solo cuando esté "PRÓXIMAMENTE" -->
      </div>
    `;
    ...
    if (status === "live") {
      ...
      card.onclick = () => {
        // Abrir el reproductor en pantalla completa horizontal tipo app
        const channel = ev.url.replace('go:', '');
        const playerURL = `https://hernanmix.github.io/reproductor/player1.html?channel=${channel}`;
        const w = screen.width;
        const h = screen.height;
        window.open(playerURL, '_blank', `toolbar=no,scrollbars=no,resizable=yes,top=0,left=0,width=${w},height=${h}`);
      };
    } else {
  ...
  fetch("channels.json")
    ...
    .then(data => {
      ...
      data.forEach(ch => {
        ...
        card.onclick = () => {
          // Abrir el reproductor en pantalla completa horizontal tipo app
          const channel = ch.url.replace('go:', '');
          const playerURL = `https://hernanmix.github.io/reproductor/player1.html?channel=${channel}`;
          const w = screen.width;
          const h = screen.height;
          window.open(playerURL, '_blank', `toolbar=no,scrollbars=no,resizable=yes,top=0,left=0,width=${w},height=${h}`);
